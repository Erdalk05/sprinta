...
Günümüzde elektrik; aydınlatmadan haberleşmeye, ulaşımdan sağlığa kadar her alanda hayatı kolaylaştırmaktadır. Yenilenebilir enerji kaynakları kullanılarak daha temiz elektrik üretme çalışmaları hız kazanmakta; bu alan gelecekte de önemini koruyacaktır.$BODY$ WHERE title = 'Elektrik ve Manyetizma';
...
-- LGS Matematik 2
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM text_library WHERE title = 'Olasılık ve İstatistik') THEN
  INSERT INTO text_library (title, body, exam_type, category, difficulty, word_count, status, estimated_read_time)
  VALUES (
...
-- LGS İngilizce
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM text_library WHERE title = 'Technology in Our Daily Lives') THEN
  INSERT INTO text_library (title, body, exam_type, category, difficulty, word_count, status, estimated_read_time)
  VALUES (
