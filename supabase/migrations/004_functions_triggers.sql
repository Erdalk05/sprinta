...
DECLARE
  v_full_name TEXT;
  v_exam_target public.exam_type;
  v_grade_level public.grade_level;
BEGIN
  v_full_name    := COALESCE(NEW.raw_user_meta_data->>'full_name', 'Öğrenci');
  v_exam_target  := COALESCE((NEW.raw_user_meta_data->>'exam_target')::public.exam_type, 'tyt');
  v_grade_level  := COALESCE((NEW.raw_user_meta_data->>'grade_level')::public.grade_level, 'lise_11');

  IF NEW.raw_user_meta_data->>'role' IS NULL OR
     NEW.raw_user_meta_data->>'role' = 'student' THEN

    INSERT INTO public.students (auth_user_id, email, full_name, exam_target, grade_level)
    VALUES (NEW.id, NEW.email, v_full_name, v_exam_target, v_grade_level)
    ON CONFLICT (auth_user_id) DO NOTHING;

    INSERT INTO public.cognitive_profiles (student_id)
    SELECT id FROM public.students WHERE auth_user_id = NEW.id
