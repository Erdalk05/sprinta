 ...
 * Her 7. kelime boşluk → 4 şık seç → LGS okuma sorusu formatı
 */
...
const PASSAGES = [
  {
    ...
    options: [
      ['dörtte birini', 'yarısını', 'üçte birini', 'beşte birini'],
      ['bitkiler', 'yüzyıllar', 'gezegenler', 'dağlar'],
      ['üretmekte', 'emerek', 'dağıtmakta', 'artırmakta'],
      ['erozyonu', 'sıcaklığı', 'yağışı', 'rüzgarı'],
    ],
    answers: [2, 0, 1, 0],  // index in opts for correct answer, mapped to every-7th-word positions
  },
]

function makeCloze(text: string, interval = 7): { parts: {word: string; isBlank: boolean; blankIdx: number}[] } {
  const words = text.split(/\s+/)
  let blankIdx = 0
  const parts = words.map((word, i) => {
    const isBlank = (i + 1) % interval === 0
    if (isBlank) {
      const idx = blankIdx
      blankIdx++
      return { word, isBlank: true, blankIdx: idx }
    }
    return { word, isBlank: false, blankIdx: -1 }
  })
  return { parts }
}
...
export default function ClozeTestScreen({ onExit }: Props) {
  ...
  const { parts } = useMemo(() => makeCloze(passage.text), [])
  const blanks = parts.filter(p => p.isBlank)
  const blankCount = Math.min(blanks.length, passage.options.length)
  ...
  const handleSubmit = useCallback(async () => {
    ...
    for (let i = 0; i < blankCount; i++) {
      if (selected[i] === passage.answers[i]) s++
    }
    setScore(s)
    if (student?.id) {
      try {
    ...
    setPhase('result')
  }, [selected, blankCount, passage, student])

  if (phase === 'result') {
    return (
      <SafeAreaView style={s.root}>
        <View style={s.centerWrap}>
          <Text style={s.emoji}>{score >= 3 ? '🏆' : score >= 2 ? '👏' : '💪'}</Text>
      ...
      <ScrollView contentContainerStyle={s.scroll}>
        ...
        <View style={s.textCard}>
          <View style={s.inlineWrap}>
            {parts.map((p, i) => {
              if (!p.isBlank || p.blankIdx >= blankCount) {
              ...
              return (
                ...
                >
                  <Text style={[s.blankTxt, isFilled && s.blankTxtFilled]}>
                    {isFilled ? passage.options[p.blankIdx][sel] : '___'}
                  </Text>
                </TouchableOpacity>
              )
            })}
        ...
        {activeBlank !== null && activeBlank < blankCount && (
          <View style={s.optPanel}>
            <Text style={s.optPanelTitle}>Boşluk {activeBlank + 1} seçeneği:</Text>
            {passage.options[activeBlank].map((opt, oi) => (
              <TouchableOpacity
                key={oi}
