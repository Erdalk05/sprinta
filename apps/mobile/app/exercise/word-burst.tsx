...
export default function uworduburstScreen() {
    ...
    if (student?.id) {
      try {
        await (supabase as any).from('reading_mode_sessions').insert({
          student_id: student.id, mode: 'word_burst', avg_wpm: m.avgWPM,
  ...
  if (!started) return <ReadingModuleIntro moduleKey="word-burst" onStart={() => setStarted(true)} onBack={() => router.back()} />
