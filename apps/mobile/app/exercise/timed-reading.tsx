...
export default function utimedureadingScreen() {
    ...
    if (student?.id) {
      try {
        await (supabase as any).from('reading_mode_sessions').insert({
          student_id: student.id, mode: 'timed', avg_wpm: m.avgWPM,
  ...
  if (!started) return <ReadingModuleIntro moduleKey="timed-reading" onStart={() => setStarted(true)} onBack={() => router.back()} />
