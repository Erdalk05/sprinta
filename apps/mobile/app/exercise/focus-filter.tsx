...
export default function ufocusufilterScreen() {
    ...
    if (student?.id) {
      try {
        await (supabase as any).from('reading_mode_sessions').insert({
          student_id: student.id, mode: 'focus_filter', avg_wpm: m.avgWPM,
  ...
  if (!started) return <ReadingModuleIntro moduleKey="focus-filter" onStart={() => setStarted(true)} onBack={() => router.back()} />
