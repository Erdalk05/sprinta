...
export default function uspeeduladderScreen() {
    ...
    if (student?.id) {
      try {
        await (supabase as any).from('reading_mode_sessions').insert({
          student_id: student.id, mode: 'speed_ladder', avg_wpm: m.avgWPM,
  ...
  if (!started) return <ReadingModuleIntro moduleKey="speed-ladder" onStart={() => setStarted(true)} onBack={() => router.back()} />
