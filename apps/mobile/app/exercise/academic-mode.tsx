...
export default function uacademicumodeScreen() {
    ...
    if (student?.id) {
      try {
        await (supabase as any).from('reading_mode_sessions').insert({
          student_id: student.id, mode: 'academic', avg_wpm: m.avgWPM,
  ...
  if (!started) return <ReadingModuleIntro moduleKey="academic-mode" onStart={() => setStarted(true)} onBack={() => router.back()} />
