...
export default function umemoryuanchorScreen() {
    ...
    if (student?.id) {
      try {
        await (supabase as any).from('reading_mode_sessions').insert({
          student_id: student.id, mode: 'memory', avg_wpm: m.avgWPM,
  ...
  if (!started) return <ReadingModuleIntro moduleKey="memory-anchor" onStart={() => setStarted(true)} onBack={() => router.back()} />
