...
export default function usentenceustepScreen() {
    ...
    if (student?.id) {
      try {
        await (supabase as any).from('reading_mode_sessions').insert({
          student_id: student.id, mode: 'sentence_step', avg_wpm: m.avgWPM,
  ...
  if (!started) return <ReadingModuleIntro moduleKey="sentence-step" onStart={() => setStarted(true)} onBack={() => router.back()} />
