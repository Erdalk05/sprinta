'use client'
// =====================================================
// app/admin/texts/[id]/chapters/[chapterId]/edit/page.tsx
// Sprint 6: Bölüm düzenle
// =====================================================
...
import type { UpdateChapterInput, ExamType, AiGenerationResponse, TextChapter } from '@sprinta/api'
...
export default function EditChapterPage({ params }: Props){
  const { id: textId, chapterId } = use(params)
  ...
  useEffect(() => {
    ...
    async function load(): Promise<void> {
      const { data } = await libSvc.getChapterById(chapterId)
      if (data) {
        setChapter(data)
        setTitle(data.title)
        setBody(data.body)
        setStatus((data as TextChapter & { status?: string }).status as 'draft' | 'published' ?? 'draft')
      }
      setLoading(false)
    }
    void load()
  ...
  async function handleAiGenerate(): Promise<void> {
    ...
    try {
      ...
      const { data, error: aiErr } = await aiSvc.generateAiMetadata({ textId, body, examType })
  ...
  async function handleSubmit(e: React.FormEvent): Promise<void> {
    ...
    try {
      ...
      const { error: updateErr } = await adminSvc.updateChapter(input)
  ...
  return (
    <div className="p-8 max-w-3xl">
      <h1 className="text-2xl font-bold text-white mb-6">Bölüm Düzenle</h1>
        ...
        <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-4">
          ...
          {aiResult && (
            <div className="space-y-2 text-xs text-slate-400">
              ...
              <p><span className="text-slate-300 font-medium">Anahtar Kelimeler:</span>{' '}{aiResult.keywords.join(', ')}</p>
              <p><span className="text-slate-300 font-medium">Sınav Etiketleri:</span>{' '}{aiResult.examTags.join(', ')}</p>
            </div>
          )}
        </div>
